...
staff = [
    {
        ...
        "supervisor": random.choice(
            [None] + [j for j in range(START_STAFF_ID, START_STAFF_ID + i)]
        ),
    }
...
inventory = [
    {
        ...
        "expirationDate": (
            datetime.date.today() + datetime.timedelta(days=random.randint(1, 90))
        ).isoformat(),
    }
...
order_alterations = [
    {"orderID": order["orderID"], "alterationsDetails": fake.sentence()}
    for order in orders
]
...
order_details = [
    {"item": item, "orderID": orderID, "quantity": random.randint(1, 5)}
...
recipe_details = [
    {"item": item, "ingredient": ingredient, "measurement": fake.word()[:255]}
...
insert_statements = {
    ...
    "MenuItemDetails": generate_sql_insert_statements(
        "MenuItemDetails", menu_item_details
    ),
    "Inventory": generate_sql_insert_statements("Inventory", inventory),
    "InventorySeasonal": generate_sql_insert_statements(
        "InventorySeasonal", inventory_seasonal
    ),
    "RestaurantTable": generate_sql_insert_statements(
        "RestaurantTable", restaurant_tables
    ),
    "RestaurantOrder": generate_sql_insert_statements("RestaurantOrder", orders),
    "OrderAlterations": generate_sql_insert_statements(
        "OrderAlterations", order_alterations
    ),
    ...
    "ReservationRequests": generate_sql_insert_statements(
